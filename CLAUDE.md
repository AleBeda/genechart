...
- **Nickname (`NICK`)**: `Individual.nickname` is parsed from the standard GEDCOM `NICK` tag, accepted at **level 1** (flat `1 NICK`) **or level 2** (`2 NICK` under `NAME`). The tag is matched **literally** in `src/parser/mod.rs` (a deliberate exception to the "no hardcoded tag strings" rule, which targets non-standard/configurable tags) — NICK is *also* still pushed onto the per-record `unparsed` buffer when a plugin is active, so the Lua plugin example that reads `unparsed` keeps working. `format_name()` (in `src/format.rs`) adds a `{nickname}` variable and, when `show.nickname` is true and the nickname is non-empty, formats with `format.individual_nickname` instead of `format.individual`. Merge fills `nickname` from a further record (`merge_into`).
- **SVG CSS classes**: every SVG element carries a `class=` attribute so charts can be restyled without touching TOML. The SVG backend helper `class_for_attrs(attrs: TextAttr) → &str` maps `TextAttr` variants to class names (`indi_name`, `spouse_name`, `indi_birth`, `indi_death`, `indi_marriage`, `indi_id`, `gen_num`, `note_text`; highlighted text also adds `highlighted`). Non-text elements use fixed class names: boxes → `box` (or `box double` for two-spouse boxes), connectors → `connector`, wedges → `wedge`, row rules → `row_rule`, note bars → `note_bar`, note links → `note_link`, highlight backgrounds → `highlight_rect`, photos → `photo`, title text → `title`, copyright text → `copyright`. Functions `svg_text_full`, `svg_line`, `svg_rect`, `render_mixed_text`, and `render_mixed_text_rotated` all accept a `class: &str` parameter.
...
- **`output.style.spacing.names_autocompress`** (default `0.85`): `boxes`/`boxed_couples` only. Implemented entirely in `src/backend/svg.rs`'s `Primitive::Text` arm of `render_bc_primitive` (gated by `BcSvgCtx.box_name_autocompress = is_boxes() || is_boxed_couples()`). For an `IndividualName`/`SpouseName`, it measures the name via `mixed_text_width` (same U+2000-split metric used by `render_mixed_text`), compares to `bbox.w − 2·NAME_CLEARANCE` (the box/section width), and sets `x_scale = max(available/measured, floor)` when too wide and `pref < 1.0`. A compressed name is wrapped in `<g transform="translate(ax,0) scale(sx,1) translate(-ax,0)">` (anamorphic X-scale about the box centre `ax`; proven through svg2pdf like the fan's rotate). The person id comes from the **threaded `current_id`** (nearest non-empty ancestor group id, `-name` suffix stripped) — no `TextPrimitive` field added. Diagnostics: `info` when shrunk (id, name, `%`), `warning` when it still overflows (id, name, overflow `%`), printed info-then-warning.
- **`output.style.wedges`**: fan-layout wedge styling (`width`, `border`, `background`) with the same semantics as `[output.style.boxes]`. `WedgeStylePrefs` in `src/preferences.rs` (mirrors `BoxStylePrefs` minus `radius`); `render_scene` in `src/backend/svg.rs` computes `wedge_fill`/`wedge_stroke`/`wedge_sw` (default width `0.5`) and the `Primitive::Wedge` arm uses them instead of reusing the box style. `WedgePrimitive` carries geometry only — styling is applied at the backend from prefs.
- **Color hex + alpha**: `hex_color(val: i64)` in `src/backend/svg.rs` is the single converter for all pref colors (except `realistic_tree`, which keeps its own `#{:06X}`). Width is inferred by magnitude: `≤0xFFF` → 3-digit → `#RRGGBB` (opaque); `≤0xFFFF` → 4-digit RGBA, **alpha-last**, → `#RRGGBBAA`; `≤0xFFFFFF` → 6-digit → `#RRGGBB`; else 8-digit → `#RRGGBBAA`. `usvg`/`svg2pdf` parse hex-with-alpha, so transparency works in SVG and PDF with no per-element opacity attributes. Limitation: a leading-zero-nibble color collapses to the next-smaller width (translucent black is unrepresentable).
- **Text colors (`color_for_attr`)**: `color_for_attr(attrs, prefs)` in `src/backend/svg.rs` is the single resolver for all text fill colors (every layout's `Primitive::Text`, fan wedge labels, etc.). It maps each `TextAttr` group to its `output.style.text.*` color: `IndividualName`/`SpouseName` → `names`, `BirthData`/`DeathData`/`MarriageData` → `dates`, `GenerationNum` → `gen_numbers`, `NoteText` → `notes`, `IndividualId` → `id`; highlighted text uses `highlights.color`. Each color goes through `hex_color` (so all 3/4/6/8-digit forms + alpha), with `0 ⇒ "black"` fallback so default output stays byte-identical (`fill="black"`, not `#000000`). `id` skips the guard (its default `0xE00` is non-zero). The `0 ⇒ "black"` fallback is the shared helper `color_or_black(i64)` (next to `hex_color`). The remaining text/line colors are applied directly at their render sites, not via `color_for_attr`: `text.title`/`text.copyright` (via `color_or_black`, classes `title`/`copyright`), `text.row_rule`, `text.note_bar`, `text.note_link` (via `hex_color`; their non-zero defaults `0xCCC`/`0xCCC`/`0x06C` reproduce the former `#CCCCCC`/`#cccccc`/`#0066CC`). The `fancy` layout's connector color also uses the shared `hex_color` (it previously had a 3-digit-only `hex_color_fancy`, now removed).
- **`diagnostics.errors` gating**: in `src/main.rs::run`, the output pipeline (everything after preferences are fully resolved — realistic-tree validation, parse/merge, scope, layout, render) runs inside an immediately-invoked closure; its `Err` is printed (`eprintln!("Error: …")`) **only if `prefs.diagnostics.errors`** before `process::exit(1)`. Errors raised earlier (CLI parse, GEDCOM path resolution, preference loading) propagate to `main()` and are always reported. `diagnostics.warnings` gates warnings at a few sites; `diagnostics.info`/`debug` are reserved (defined, not yet read).
...
- **Single-spouse parent centre = first/last child mean** (queue #102): a single-spouse parent's natural centre is `(first_child.x + last_child.x) / 2` (the midpoint of the outermost children), **not** the median child. The identical formula is used in `place_descendants` (the `1 =>` arm) and `recenter_pass` (the single-spouse branch) of `boxed_couples.rs`, and in `place_descendants` + `recenter_pass_desc` of `boxes.rs`. Equivalent to the old median for 1–2 children or evenly-spaced children; for 3+ with uneven subtrees it removes the lopsided parent. The 2-spouse (connector-offset) and 3-spouse (children-of-middle-spouse) centring and `boxes` ancestors mode are unchanged. A future refinement (approach B) would centre over the whole subtree's left/right envelope.
